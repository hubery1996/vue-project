				...
				<el-form-item label="商品图片">
					<el-upload class="avatar-uploader" :headers="headers" action="/api/upload/goods/" :show-file-list="false"
					 :on-success="uploadSuccess" :before-upload="beforeUpload">
						<img width="100%" v-if="form.img_md" :src="form.img_md" class="avatar">
...
<script>
	...
	export default {
		...
		computed: mapState({
			...
			editeGood: state => state.wangEditor.editeGood,
		...
		methods: {
			...
			Category_lst() {
				...
				var info = hash.indexOf('=');
				var str = hash.slice(info + 1)
				...
				this.$http.get('/api/goods/detail', {
					...
					console.log(res);
					this.$store.commit('editeDetailMut', {
						amount: res.data.detail
					})
					this.form.discount = res.data.discount;
					this.form.hotPoint = res.data.hotPoint;
					this.form.id = res.data.id;
					this.form.marketPrice = res.data.marketPrice;
					this.form.name = res.data.name;
					this.form.price = res.data.price;
					this.form.slider = res.data.slider;
					this.form.detail = this.editeDetail;
					// 从仓库取数据中的图片
					this.form.img_md = this.editeGood.img_md;
					console.log(this.form);
				});
			},
			Category_2st() {
